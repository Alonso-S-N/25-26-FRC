...
public class SwerveSub extends SubsystemBase {
  ...
  public final NetworkTable RobotDetector;
  ...
  public SwerveSub() {
    ...
    RobotDetector = NetworkTableInstance.getDefault().getTable("robot-detector");
...
public void cancelSnap() {
  ...
  public void periodic() {
    // ============================SwervePath=============================//
  List<Translation2d> opponents = getDetectedOpponents();
  List<Pair<Translation2d, Translation2d>> obstacles = new ArrayList<>();

  double margin = 0.5; 
  for (Translation2d opp : opponents) {
    obstacles.add(new Pair<>(
        opp.plus(new Translation2d(-margin, -margin)),
        opp.plus(new Translation2d( margin,  margin))
    ));
  }

  Pathfinding.setDynamicObstacles(obstacles, getPose().getTranslation());
...
getMegaTagPose().ifPresent(p ->
  ...
  if (RobotDetector.getEntry("opp_has_target").getBoolean(false) == false) {
    return opponents;
  }

  double[] txs = RobotDetector.getEntry("opp_tx").getDoubleArray(new double[0]);
  double[] dists = RobotDetector.getEntry("opp_dist").getDoubleArray(new double[0]);

  double lastUpdate = RobotDetector.getEntry("opp_timestamp").getDouble(0);
  if (Timer.getFPGATimestamp() - lastUpdate > 0.5) {
    return new ArrayList<>();
  }

  Pose2d robotPose = getPose();

  int count = Math.min(txs.length, dists.length);
  for (int i = 0; i < count; i++) {
    double angleRad = robotPose.getRotation().getRadians() + Math.toRadians(txs[i]);

    Translation2d offset = new Translation2d(
        dists[i] * Math.cos(angleRad),
        dists[i] * Math.sin(angleRad)
    );

    opponents.add(robotPose.getTranslation().plus(offset));
  }
