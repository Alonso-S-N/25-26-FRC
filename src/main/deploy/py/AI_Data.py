...
MODEL_PATH = "C:\\Users\\Usuario\\25-26\\SwerveDrive\\src\\main\\deploy\\py\\PegaRobo.pt"
CONF_THRESHOLD = 0.50
...
# NETWORKTABLES
# =========================
init_nt(server="10.91.63.2", table_name="vision")
...
if __name__ == "__main__":
    try:
        ...
        running = False
def infer_robots(model: YOLO, frame):
    if frame is None:
        return []
    ...
    for r in results:
        for box in r.boxes:
            ...
            # distância estimada pela altura do bounding box
            # (precisa calibrar ROBOT_HEIGHT_M e focal_px pro seu setup)
            distance_m = (ROBOT_HEIGHT_M * focal_px) / bbox_height_px

            detections.append((tx_deg, distance_m))

    return detections
    
def rio2wpi_opponents(txs, distances):
    _ensure()
    _table.putNumberArray("opp_tx", [float(v) for v in txs])
    _table.putNumberArray("opp_dist", [float(v) for v in distances])
    _table.putBoolean("opp_has_target", len(txs) > 0)